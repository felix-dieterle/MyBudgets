...
# Ensure console output uses UTF-8 when possible to avoid UnicodeEncodeError on Windows
try:
    # Python 3.7+: reconfigure stdout/stderr to utf-8 with safe error handler
    if hasattr(sys.stdout, "reconfigure"):
        sys.stdout.reconfigure(encoding="utf-8", errors="backslashreplace")
    if hasattr(sys.stderr, "reconfigure"):
        sys.stderr.reconfigure(encoding="utf-8", errors="backslashreplace")
except Exception:
    # best-effort, fall back to original encoding
    pass
...
def _setup_logging(verbose: bool) -> None:
    ...
    datefmt = "%H:%M:%S"
    # Configure root logger with a StreamHandler that writes to sys.stdout (UTF-8)
    root = logging.getLogger()
    # remove existing handlers to avoid duplicate output if re-running
...
def _get_fints_data_file(blz: str, user_id: str) -> str:
    """Returns the path to the fints data blob file for this BLZ/user combination."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    data_dir = os.path.join(script_dir, "fints_data")
    os.makedirs(data_dir, exist_ok=True)
    # Sanitize filename: replace special chars with underscore
    safe_user = "".join(c if c.isalnum() else "_" for c in user_id)
    return os.path.join(data_dir, f"fints_{blz}_{safe_user}.bin")


def _load_fints_data(blz: str, user_id: str) -> Optional[bytes]:
    """Load persisted fints data blob if it exists."""
    data_file = _get_fints_data_file(blz, user_id)
    if os.path.exists(data_file):
        try:
            with open(data_file, "rb") as f:
                return f.read()
        except Exception as exc:
            LOG.warning("Konnte fints-Daten nicht laden: %s", exc)
    return None


def _save_fints_data(blz: str, user_id: str, data_blob: bytes) -> None:
    """Save fints data blob to disk."""
    data_file = _get_fints_data_file(blz, user_id)
    try:
        with open(data_file, "wb") as f:
            f.write(data_blob)
        LOG.debug("fints-Daten gespeichert: %s", data_file)
    except Exception as exc:
        LOG.warning("Konnte fints-Daten nicht speichern: %s", exc)


def _sep(title: str = "") -> None:
    width = 60
    # Use box-drawing character when the terminal encoding supports it; fallback to '-'
    box_char = "\u2500"
    try:
...
def run(args: argparse.Namespace) -> int:  # noqa: C901
    ...
    # Handle --reset-cached: delete saved fints data
    if args.reset_cached:
        data_file = _get_fints_data_file(blz, user)
        if os.path.exists(data_file):
            try:
                os.remove(data_file)
                LOG.info("Gespeicherte fints-Daten gelöscht: %s", data_file)
            except Exception as exc:
                LOG.warning("Konnte gespeicherte Daten nicht löschen: %s", exc)
        else:
            LOG.info("Keine gespeicherten fints-Daten gefunden (nichts zu löschen)")
    ...
    # If user requested only the latest booking, shorten lookback and limit output
    if getattr(args, "last_only", False):
        # shorten to at most the last 7 days to speed up the call
        lookback = min(args.days_back, 7)
        start_date = end_date - timedelta(days=lookback)
        # ensure we only print a single entry
        args.max_entries = 1
    ...
    # Try to load persisted fints data (includes system_id, BPD, etc.)
    saved_data = _load_fints_data(blz, user)
    if saved_data:
        LOG.info("[1/5] Gespeicherte fints-Daten geladen (system_id, BPD, etc.)")
    ...
    try:
        client = FinTS3PinTanClient(
            ...
            from_data=saved_data,
        )
        # For banks that don't support HKSYN3 properly (like BBBank), force system_id to '0'
        # This tells the bank we're a new client and don't have a system_id yet
        if not saved_data:
            LOG.info("[1/5] Keine gespeicherten Daten – setze system_id='0' (umgehe HKSYN3)")
            client.system_id = '0'
    except Exception as exc:
    ...
    try:
        with client:
            ...
            if not transactions:
                ...
                # If --last-only was requested, select the most recent transaction
                if getattr(args, "last_only", False) and transactions:
                    try:
                        def _tx_key(tx):
                            data = tx.data if isinstance(tx.data, dict) else {}
                            return data.get("date") or data.get("entry_date") or data.get("guessed_entry_date") or ""

                        latest = max(transactions, key=_tx_key)
                        transactions = [latest]
                    except Exception:
                        # Fallback: keep the last element
            ...
            # Save fints data (system_id, BPD, etc.) for next run
            try:
                data_blob = client.deconstruct_v1(including_private=True)
                _save_fints_data(blz, user, data_blob)
                LOG.debug("[1/5] fints-Daten für nächsten Lauf gespeichert")
            except Exception as exc:
                LOG.warning("Konnte fints-Daten nicht speichern: %s", exc)
...
def main() -> int:
    ...
    parser.add_argument(
        "--server", default="",
        help=(
            "FinTS-Server URL. Für BBBank z.B. https://fints2.atruvia.de/cgi-bin/hbciservlet. "
    ...
    parser.add_argument(
        ...
        help="Nur die letzte (neueste) Buchung anzeigen – verkürzt Laufzeit/Output",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--reset-cached", action="store_true",
        help="Lösche gespeicherte fints-Daten (system_id, BPD, etc.) und erzwinge Neusynchronisation",
    )
    ...
    # Allow credentials via environment variables for non-interactive runs.
    # WARNING: Avoid placing secrets on the command line; prefer env vars or prompts.
    import os

    if not args.iban:
        args.iban = os.environ.get("MYB_IBAN", "")
    if not args.user:
        args.user = os.environ.get("MYB_USER", "")
    if not args.pin:
        args.pin = os.environ.get("MYB_PIN", "")
